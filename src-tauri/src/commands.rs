...
fn perform_upscale(
    ...
    let resolve_model_path = |h: &tauri::AppHandle, filename: &str| -> Result<PathBuf, String> {
        h.path()
...
pub async fn upscale_image(
    ...
    let res = tauri::async_runtime::spawn_blocking({
        ...
        move || perform_upscale(&handle, &image_id, &input_path, &model_filename, base_scale, target_scale, &output_dir, &token)
    })
    .await
    .map_err(|e| {
